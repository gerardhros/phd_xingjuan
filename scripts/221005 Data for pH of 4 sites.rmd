...
d1 <- readxl::read_xlsx('../data/221005 Data for pH for 4 sites.xlsx')
d1 <- as.data.table(d1)
...
#d2 <- readxl::read_xlsx('220916 soil ph output for 4 sites.xlsx')
d2 <- copy(d1)
d2 <- as.data.table(d2)

d2[, uid := .GRP, by =c('sitename','treat')]
...
for(i in 1:max(d2$uid)){
 ...
 # d2.cumsum <- d2[year != min(year),lapply(.SD,function(x) max(cumsum(x))),.SDcols = c('n_sur','bc_sur')]
  ...
  d3.diff <- d3[,list(dph = mean(d3[(filt*2+1):nrow(d3),ph]) - quantile(d3[1:filt,ph],0.25))]
  
  
  # retreive the relevant information
  d3.out <- data.table(site = unique(d3$sitename)[1],
                       treatment = unique(d3$treat[1]),
...
# make columns with numbers also numeric
 ...
 # in paddy soils there is high denitrifcation, so low fraction of N surplus that causes acification via leaching
 d4[ct1=='P', cfn2 := 0]
 d4[ct1=='U-P', cfn2 := 0.2]
 d4[is.na(cfn2),cfn2 := 0.8]
 
 # units is eq / m2
 d4[,nbc_sur := cfn2 * n_sur - bc_sur]
 d4[,nbcr_sur := cfn2 * n_sur / bc_sur]
 d4[,nbc_sur_wd := n_sur - bc_sur]
 
 d5 = d4[,.(nsurplus = n_sur,
       bcsurplus = bc_sur,
       acid = n_sur - bc_sur,
       nbc_sur,
       alfa,dph,
       site,treatment)]
plot(alfa~nbc_sur,data=d5,pch=16,cex=2)
...
summary(lm(alfa~ct1:nbcr_sur,data=d4))
...
# make a model with crop type, N surplus, BC surplus
...
lm5 <- lm(alfa~ ct1:nbc_sur +  cec,data=d4)
